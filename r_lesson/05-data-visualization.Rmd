...
**`ggplot2`** belongs to the [**`tidyverse`** framework](https://www.tidyverse.org/). Therefore, we will start with loading the package **`tidyverse`**. If **`tidyverse`** is not already installed, then we need to install first. If the tidyverse has been installed and loaded, then we can skip the following steps:
...
```{r, echo=FALSE, eval=TRUE, purl=FALSE, message=FALSE, warning=FALSE}
## silently read in CSV
variants <- read_csv("raw_data/combined_tidy_vcf.csv")
```
